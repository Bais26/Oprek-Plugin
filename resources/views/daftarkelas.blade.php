            ...
            <form action="{{ route('calon_siswa.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="squad" id="squad" class="w-full p-3 border border-gray-300 rounded-md"
                            required>
                            <option value="" disabled {{ old('squad') ? '' : 'selected' }}>Pilih Squad</option>
                            <option value="Frontend Website"
                                {{ old('squad') == 'Frontend Website' ? 'selected' : '' }}>Frontend Website</option>
                            <option value="Mobile Kotlin" {{ old('squad') == 'Mobile Kotlin' ? 'selected' : '' }}>
                                Mobile Kotlin</option>
                            <option value="Mobile Flutter" {{ old('squad') == 'Mobile Flutter' ? 'selected' : '' }}>
                                Mobile Flutter</option>
                            <option value="Backend Developer"
                                {{ old('squad') == 'Backend Developer' ? 'selected' : '' }}>Backend Developer</option>
                            <option value="Laravel" {{ old('squad') == 'Laravel' ? 'selected' : '' }}>Laravel Developer</option>
                        </select>
                        @error('squad')
                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div>
                        <button type="submit"
                            class="w-full bg-blue-500 text-white p-3 rounded-md hover:bg-blue-600">Daftar</button>
                    </div>
                </div>
            </form>
        </div>
